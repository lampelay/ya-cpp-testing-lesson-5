...
public:
    ...
    bool AreSynonyms(const string& first_word, const string& second_word) const {
        return synonyms_.count(first_word) 
            && synonyms_.at(first_word).count(second_word);
    }
...
void TestAreSynonyms() {
    Synonyms synonyms;
    assert(!synonyms.AreSynonyms("word1"s, "word2"s));
    synonyms.Add("word1"s, "word2"s);
    assert(synonyms.AreSynonyms("word1"s, "word2"s));
    synonyms.Add("word2"s, "word3"s);
    assert(!synonyms.AreSynonyms("word1"s, "word3"s));
}
